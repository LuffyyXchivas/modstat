...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    const taggeds = members.filter((x) => x.user.username.includes(conf.tag.tag.some((x,z) => z.user.username.includes(x)))).map((x) => parseInt(x.nickname.split(conf.registration.brace)[1]));
