...
module.exports = {
    ...
    run: async (client, message, args, embed, prefix) => {
      if (["ekle", "add", "oluştur"].includes(args[0])) {
        if (!conf.botOwner == message.author.id) return message.channel.error(message, "Yetki eklemek için yeterli yetkin bulunmuyor!");
        const name = args[1];
    ...
    \`•\` \`${prefix}yetki düzenle ${name}\` komutu ile verilecek rolleri ve kullanabilecek kişileri ayarlamayı unutmayın!
        ...
        if (!conf.botOwner == message.author.id) return message.channel.error(message, "Yeterli yetkin bulunmuyor!");
